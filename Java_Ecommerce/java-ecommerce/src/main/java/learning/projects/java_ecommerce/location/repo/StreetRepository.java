...
public interface StreetRepository extends JpaRepository<Street, StreetId>{
    ...
    @Query("SELECT a FROM Street a JOIN FETCH a.city WHERE a.postalCode = :postCode")
    List<Street> findAddressesWithCity(@Param("postCode") String postCode);

    @Query("SELECT new StreetLookupDto(a.id, a.streetName) " + "FROM Street a WHERE a.city.id = :cityId")
